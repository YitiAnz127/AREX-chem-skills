...
def register(server: Any) -> None:
    ...
    @server.tool(
        name="sra_search",
        description=(
            "Search NCBI SRA sequence read archive (raw sequencing data). "
            "检索 NCBI SRA 序列读取档案：按基因/物种/测序策略（如 RNA-seq[Strategy] AND cancer），"
            "returns run accession/study/sample/organism/strategy/platform. "
            "返回测序运行编号、研究、样本、物种、策略与平台，用于查找原始测序数据。"
        ),
    )
    def sra_search(
        query: str,
        ...
        Args:
            query: 检索词（如 BRCA1 AND Homo sapiens[Organism]）。
            max_results: 返回条数（1-10）。
        """
        max_results = max(1, min(int(max_results), 10))
        client = NCBIClient()
        try:
            data = client.sra_search(query, max_results)
        finally:
            client.close()
        if not data["runs"]:
            return f"NCBI SRA 未找到「{query}」。"
        lines = [f"# SRA 测序数据检索「{query}」（共 {data['count']} 条）", ""]
        for r in data["runs"]:
            lines.append(f"- **{r['accession']}**: {r['title'] or r['study']}")
            parts = []
            if r.get("organism"):
                parts.append(r["organism"])
            if r.get("strategy"):
                parts.append(r["strategy"])
            if r.get("platform"):
                parts.append(r["platform"])
            if parts:
                lines.append(f"  {' · '.join(parts)}")
        lines.append("")
        lines.append("来源：NCBI SRA（序列读取档案）。")
        return "\n".join(lines)

    @server.tool(
        name="bioproject_search",
        description=(
            "Search NCBI BioProject biological projects. "
            "检索 NCBI BioProject 生物项目：按主题/项目编号（如 gut microbiome AND human、PRJNA123456），"
            "returns project accession/title/description/organism. "
            "返回项目编号、标题、描述与物种，用于关联大规模测序项目与数据集。"
        ),
    )
    def bioproject_search(
        query: str,
        max_results: int = 5,
    ) -> str:
        """检索 BioProject。
        ...
        Args:
            query: 检索词（如 gut microbiome、PRJNA123456）。
            max_results: 返回条数（1-10）。
        """
        max_results = max(1, min(int(max_results), 10))
        client = NCBIClient()
        try:
            data = client.bioproject_search(query, max_results)
        finally:
            client.close()
        if not data["projects"]:
            return f"NCBI BioProject 未找到「{query}」。"
        lines = [f"# BioProject 检索「{query}」（共 {data['count']} 条）", ""]
        for p in data["projects"]:
            lines.append(f"- **{p['accession']}**: {p['title']}")
            if p.get("organism"):
                lines.append(f"  物种: {p['organism']}")
            if p.get("description"):
                lines.append(f"  描述: {p['description'][:200]}")
        lines.append("")
        lines.append("来源：NCBI BioProject。")
        return "\n".join(lines)
