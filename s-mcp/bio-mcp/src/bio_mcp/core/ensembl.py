...
class EnsemblClient:
    ...
    def gene_by_symbol(self, symbol: str, species: str = "homo_sapiens") -> dict[str, Any]:
        ...
        若 symbol 端点未命中（例如植物 locus ID 如 AT1G01010 并非显示符号），
        自动回退到 lookup/id 端点按基因 ID 查询。
        """
        try:
            resp = self._http.get(f"lookup/symbol/{species}/{symbol}")
            data = resp.json()
        except Exception:
            # symbol 查询失败（如植物 locus ID 非显示符号）时回退 lookup/id
            data = {}
        if not isinstance(data, dict) or not data.get("id"):
            try:
                resp2 = self._http.get(f"lookup/id/{symbol}")
                data2 = resp2.json()
