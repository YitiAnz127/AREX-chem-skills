...
class NCBIClient:
    ...
    # ---- SRA：序列读取档案检索 ----
    def sra_search(self, term: str, retmax: int = 5) -> dict[str, Any]:
        """检索 NCBI SRA 序列读取档案（测序原始数据）。

        term 示例：
          "BRCA1 AND Homo sapiens[Organism]"
          "RNA-seq[Strategy] AND cancer"
        """
        es = self.esearch("sra", term, retmax=retmax)
        ids = es["ids"]
        if not ids:
            return {"count": 0, "runs": []}
        runs: list[dict[str, Any]] = []
        for rec in self.esummary("sra", ids):
            runs.append(
                {
                    "accession": rec.get("runacc") or rec.get("acc", ""),
                    "study": rec.get("study_name", ""),
                    "sample": rec.get("sample_name", ""),
                    "organism": rec.get("organism", ""),
                    "strategy": rec.get("strategy", ""),
                    "platform": rec.get("platform", ""),
                    "title": (rec.get("expxml") or "").strip()[:200],
                    "taxid": rec.get("taxid"),
                }
            )
        return {"count": es["count"], "runs": runs}

    # ---- BioProject：生物项目检索 ----
    def bioproject_search(self, term: str, retmax: int = 5) -> dict[str, Any]:
        """检索 NCBI BioProject 生物项目。

        term 示例：
          "gut microbiome AND human"
          "PRJNA123456"
        """
        es = self.esearch("bioproject", term, retmax=retmax)
        ids = es["ids"]
        if not ids:
            return {"count": 0, "projects": []}
        projects: list[dict[str, Any]] = []
        for rec in self.esummary("bioproject", ids):
            projects.append(
                {
                    "accession": rec.get("project_acc", ""),
                    "title": (rec.get("project_title") or "").strip(),
                    "description": (rec.get("project_description") or "").strip()[:300],
                    "organism": (rec.get("organism") or "").strip(),
                    "date": rec.get("project_data_to", ""),
                    "uid": rec.get("uid"),
                }
            )
        return {"count": es["count"], "projects": projects}

    # ---- BioSample：生物样本检索（E-utilities db=biosample）----
    def biosample_search(self, term: str, retmax: int = 5) -> dict[str, Any]:
        """检索 NCBI BioSample 生物样本。

        term 示例：
          "Homo sapiens[Organism] AND blood"
          "SAMN00000001"
        """
        es = self.esearch("biosample", term, retmax=retmax)
        ids = es["ids"]
        if not ids:
            return {"count": 0, "samples": []}
        samples: list[dict[str, Any]] = []
        for rec in self.esummary("biosample", ids):
            samples.append(
                {
                    "accession": rec.get("accession", ""),
                    "title": (rec.get("title") or "").strip()[:200],
                    "organism": rec.get("taxonomy_name", "") or rec.get("organism", ""),
                    "status": rec.get("status", ""),
                    "date": rec.get("publication_date", ""),
                    "uid": rec.get("uid"),
                }
            )
        return {"count": es["count"], "samples": samples}

    # ---- Genome：全基因组序列检索（wgs）----
    def genome_search(self, term: str, retmax: int = 5) -> dict[str, Any]:
        """检索 NCBI 全基因组序列（wgs）。
        ...
        """
        es = self.esearch("wgs", term, retmax=retmax)
        ids = es["ids"]
        if not ids:
            return {"count": 0, "genomes": []}
        genomes: list[dict[str, Any]] = []
        for rec in self.esummary("wgs", ids):
            genomes.append(
                {
                    "accession": rec.get("prefix", ""),
                    "title": (rec.get("title") or "").strip(),
                    "organism": rec.get("organism", ""),
                    "taxid": rec.get("taxid"),
                    "length": rec.get("slen"),
        ...
        return {"count": es["count"], "genomes": genomes}
